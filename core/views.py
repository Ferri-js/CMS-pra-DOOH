...
def home(request):
    ...
    playlist = (
        Playlist.objects.filter(ativa=True).prefetch_related("itens__midia").first()
    )
...
def gerenciar_midia(request):
    ...
    if request.method == "POST":
        ...
        )  # Adicionado request.FILES para uploads reais
        ...
        if form.is_valid():
            ...
            if request.headers.get("Hx-Request"):
                ...
                return render(
                    request, "core/partials/midia_list.html", {"midias": midias}
                )
